...
# Create the plot
plot(year_interest$DT_MST, year_interest$Northwest, type='l', xlab="Date", ylab="Northwest",
     main="Northwest Values over January 2011", xaxt='n') # Suppress x-axis

# Add daily ticks on the x-axis
axis.Date(1, at=seq(min(year_interest$DT_MST), max(year_interest$DT_MST), by="day"), 
...
# Plot the data
plot(aeso.nw$DT_MST, aeso.nw$Northwest, type='l',
...
new_column_names <- c("township","date","avg_temp", "min_temp", "max_temp",
...
meteo_data <- temperature[c("township","date","avg_temp", "min_temp", "max_temp",
...
# Plot avg_temp
plot(meteo_data$date,meteo_data$noisy_temp, type = 'l', col = 'red', 
...
# Add a legend
legend("topright", legend = c("Avg Temp", "Noisy Temp"), 
...
# Plotting
plot(meteo_data_year$date, meteo_data_year$avg_temp, type = 'l', col = 'blue', 
