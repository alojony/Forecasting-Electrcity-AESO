...
NAtoNeighborAverage <- function(x) {
  ...
  if (is.na(x[n]))
    x[n] <- x[max(which(!is.na(x)))] # Backward fill
...
full_set <-
  transform(
    ...
    Edmonton = as.numeric(Edmonton),
    Calgary = as.numeric(Calgary),
...
full_set <-
  subset(full_set,
         select = -c(South, Northeast, Edmonton, Calgary, Central))
